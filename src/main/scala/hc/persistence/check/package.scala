...
package object check {
  ...
  object CheckPersistence {
    trait Service {
      ...
      def getByUrlAndPeriod(url: String, beginDate: Date, endDate: Date): Task[Seq[Check]]
    }
    ...
    def getByUrlAndPeriod(url: String, beginDate: Date, endDate: Date): RIO[CheckPersistence, Seq[Check]] =
      RIO.accessM(_.get.getByUrlAndPeriod(url, beginDate, endDate))
  }
}
